...
const ProductDetailsCart = ({ data, setOpen }) => {
  ...
  return (
    <div className=" ">
      {data ? (
        <div className="w-full h-screen fixed top-0 left-0 bg-white text-gray-700 flex items-center justify-center z-50">
          <motion.div
            initial={{ opacity: 0, y: -500 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -500 }}
            className="w-[90%] 800px:w-[60%] h-[90vh] 800px:h-[75vh] rounded-md p-4 relative shadow-sm overflow-y-scroll bg-white text-gray-700  ">
            <RxCross1
              size={40}
              color="#333"
            ...
            <div className="block w-full 800px:flex">
              <div className="800px:w-[50%] w-full">
                ...
                <div className="flex items-center mt-5">
                  <img
                    src={data?.brand?.shop_avatar.url}
                    alt="shopAvatar/image"
                    className="w-[50px] h-[50px] rounded-full mr-2"
                  />
                  <div>
                    <h3 className={`${styles.shop_name} `}>{data.brand.name}</h3>
                    <h3 className="text-[15px] text-[#F85506]">({data?.rating}) ratings</h3>
                  </div>
                </div>
                <div
              ...
              <div className="w-full 800px:w-[50%] pt-8 pl-[5px] pr-[5px]">
                <h1 className={`${styles.productTitle} text-[20px] text-gray-700`}>
                  {data?.name}
                </h1>
